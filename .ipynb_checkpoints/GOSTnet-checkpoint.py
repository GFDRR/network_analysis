...
def pandana_snap(G, point_gdf, geometry_column = 'geometry', source_crs = 'epsg:4326', target_crs = 'epsg:4326', add_dist_to_node_col = False):
    ...
    if add_dist_to_node_col is True:
        ...
        in_df['Proj_geometry'] = in_df.apply(lambda x: transform(project_WGS_UTM, x[geometry_column]), axis = 1)
        in_df = in_df.set_geometry('Proj_geometry')
        in_df['x'] = in_df.Proj_geometry.x
        in_df['y'] = in_df.Proj_geometry.y

        node_gdf['Proj_geometry'] = node_gdf.apply(lambda x: transform(project_WGS_UTM, x[geometry_column]), axis = 1)
        ...
        in_df['x'] = in_df[geometry_column].x
        in_df['y'] = in_df[geometry_column].y
        G_tree = spatial.KDTree(node_gdf[['x','y']].as_matrix())
